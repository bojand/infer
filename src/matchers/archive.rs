...
pub fn is_gz(buf: &[u8]) -> bool {
    return buf.len() > 2 && buf[0] == 0x1F && buf[1] == 0x8B && buf[2] == 0x8;
}

/// Returns whether a buffer is a bzip archive.
pub fn is_bz2(buf: &[u8]) -> bool {
    return buf.len() > 2 && buf[0] == 0x42 && buf[1] == 0x5A && buf[2] == 0x68;
}
...
pub fn is_pdf(buf: &[u8]) -> bool {
    return buf.len() > 3 && buf[0] == 0x25 && buf[1] == 0x50 && buf[2] == 0x44 && buf[3] == 0x46;
}

/// Returns whether a buffer is a SWF.
pub fn is_swf(buf: &[u8]) -> bool {
    return buf.len() > 2 && (buf[0] == 0x43 || buf[0] == 0x46) && buf[1] == 0x57 && buf[2] == 0x53;
}
...
pub fn is_nes(buf: &[u8]) -> bool {
    return buf.len() > 3 && buf[0] == 0x4E && buf[1] == 0x45 && buf[2] == 0x53 && buf[3] == 0x1A;
}

/// Returns whether a buffer is Google Chrome Extension
pub fn is_crx(buf: &[u8]) -> bool {
    return buf.len() > 3 && buf[0] == 0x43 && buf[1] == 0x72 && buf[2] == 0x32 && buf[3] == 0x34;
}
...
pub fn is_ps(buf: &[u8]) -> bool {
    return buf.len() > 1 && buf[0] == 0x25 && buf[1] == 0x21;
}
...
pub fn is_lz(buf: &[u8]) -> bool {
    return buf.len() > 3 && buf[0] == 0x4C && buf[1] == 0x5A && buf[2] == 0x49 && buf[3] == 0x50;
}

/// Returns whether a buffer is an RPM.
pub fn is_rpm(buf: &[u8]) -> bool {
    return buf.len() > 96 && buf[0] == 0xED && buf[1] == 0xAB && buf[2] == 0xEE && buf[3] == 0xDB;
}
