...
pub fn is_midi(buf: &[u8]) -> bool {
    return buf.len() > 3 && buf[0] == 0x4D && buf[1] == 0x54 && buf[2] == 0x68 && buf[3] == 0x64;
}
...
pub fn is_mp3(buf: &[u8]) -> bool {
    return buf.len() > 2
        && ((buf[0] == 0x49 && buf[1] == 0x44 && buf[2] == 0x33) // ID3v2
			...
			|| (buf[0] == 0xFF && (buf[1] == 0xfb || buf[1] == 0xFB)));
}
...
pub fn is_ogg(buf: &[u8]) -> bool {
    return buf.len() > 3 && buf[0] == 0x4F && buf[1] == 0x67 && buf[2] == 0x67 && buf[3] == 0x53;
}

/// Returns whether a buffer is FLAC data.
pub fn is_flac(buf: &[u8]) -> bool {
    return buf.len() > 3 && buf[0] == 0x66 && buf[1] == 0x4C && buf[2] == 0x61 && buf[3] == 0x43;
}
...
pub fn is_aac(buf: &[u8]) -> bool {
    return buf.len() > 1
        && ((buf[0] == 0xFF && buf[1] == 0xF1) || (buf[0] == 0xFF && buf[1] == 0xF9));
}
