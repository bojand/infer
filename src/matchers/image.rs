...
/// Returns whether a buffer is OpenRaster (ora) image data.
...
// is_isobmff checks whether the given buffer represents ISO Base Media File Format data.
...
super::build_fn_read_api!(
    ...
    /// Returns whether data from reader is OpenRaster (ora) image data.
    (is_ora_read, is_ora, 58)
);
