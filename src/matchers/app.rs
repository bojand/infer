...
/// ```rust
/// use std::fs;
/// assert!(infer::matchers::app::is_wasm(&fs::read("src/testdata/sample.wasm").unwrap()));
/// ```
...
/// ```rust
/// use std::fs;
/// assert!(infer::matchers::app::is_exe(&fs::read("src/testdata/sample.exe").unwrap()));
/// ```
...
pub fn is_elf(buf: &[u8]) -> bool {
	return buf.len() > 52 && buf[0] == 0x7F && buf[1] == 0x45 && buf[2] == 0x4C && buf[3] == 0x46;
}
...
pub fn is_llvm(buf: &[u8]) -> bool {
	return buf.len() >= 2 && buf[0] == 0x42 && buf[1] == 0x43;
}